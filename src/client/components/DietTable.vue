<template lang="pug">
  .diet-table
    v-data-table.diet-table__table(
      ...
      :headers="headers"
      :no-data-text="$t('components.dietTable.noData')"
      :no-results-text="$t('components.dietTable.noResults')"
      :loading-text="$t('components.dietTable.loading')"
      ...
      template(v-slot:item.serving.value="{ item }")
        v-chip.grey.lighten-4(
          label
          link
        ) {{ formatNumber(item.serving.value) }} {{ $tc(`common.units.${item.serving.unit}`, item.serving.value)}}
      template(
        ...
        template(v-if="!readonly")
          v-edit-dialog(
            ...
            v-chip.diet-table__table__amount-display.grey.lighten-2(
              label
              link
            ) {{ formatNumber(item.amount) }} {{ $tc(`common.units.${item.serving.unit}`, item.amount)}}
        ...
        template(v-else) {{ formatNumber(item.amount) }} {{ $tc(`common.units.${item.serving.unit}`, item.amount)}}
...
export default {
  ...
  data () {
    return {
      ...
      showRecipes: true,
      headers: [
        { text: this.$t('components.dietTable.name'), value: 'name', align: 'left', width: '15rem' },
        ...(this.withAmount
          ? [{ text: this.$t('components.dietTable.amount'), value: 'amount', width: '9rem' }] : []
        ),
        ...(!this.withoutServing
          ? [{ text: this.$t('components.dietTable.serving'), value: 'serving.value', width: '9rem' }] : []
        ),
        { text: this.$t('components.dietTable.calories'), value: 'nutrients.calories', width: '7rem' },
        { text: this.$t('components.dietTable.carbs'), value: 'nutrients.carbs', width: '7rem' },
        { text: this.$t('components.dietTable.protein'), value: 'nutrients.protein', width: '7rem' },
        { text: this.$t('components.dietTable.fat',), value: 'nutrients.fat', width: '7rem' },
      ],
      footer: {
        itemsPerPageAllText: this.$t('components.dietTable.itemsPerPageAll'),
        itemsPerPageText: this.$t('components.dietTable.itemsPerPage'),
      },
    };
  },
  computed: {
